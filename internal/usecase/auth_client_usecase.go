...
func (u *authClientUsecase) Register(c *gin.Context) {
	...
	if err := u.Repo.CreateCliente(cliente); err != nil {
		fmt.Printf("error al crear el cliente: %+v", err)
	...
	c.JSON(http.StatusCreated, gin.H{"success": true, "message": "cliente registrado", "data": token})
}
